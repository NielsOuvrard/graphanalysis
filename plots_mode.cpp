        ...
        int deep)
{
    ...
    for (int i = 0; i < friendships.size(); i++) {
        ...
        //        std::cout << " f[id_queen][i] = " << friendships[id_queen][i];
        //        std::cout << " f[i][target] = " << friendships[i][target] << std::endl;
    ...
    for (int i = 0; i < friendships.size(); i++) {
        ...
        if (!VALUE_EXISTS(visited, i) && i != target && friendships[target][i] > 0) {
            ...
            for (int j = 0; j < friendships.size(); j++) {
                if (!VALUE_EXISTS(visited, j) && target != j && i != j && matrix_plots[j][i] == 1) {
